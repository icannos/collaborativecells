...
class AbstractMaddpgTrainer:
    ...
    def __init__(self, session, scope, env, nb_agent=3, agent_class=None, memory_size=10 ** 6, batch_size=1024,
                 gamma=0.99,
                 horizon=100):
        """
    ...
    def train(self, episode=1, render=True):
        ...
        for _ in range(episode):
            ...
            for d in range(self.horizon):
                ...
                if len(self.buffer.memory) < 3*self.buffer.batch_size or last_train < 200:
                    last_train += 1
                    continue
